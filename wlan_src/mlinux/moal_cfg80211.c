...
woal_clear_all_mgmt_ies(moal_private *priv, t_u8 wait_option)
{
	...
	if (priv->proberesp_p2p_index != MLAN_CUSTOM_IE_AUTO_IDX_MASK)
		mask |= MGMT_MASK_PROBE_RESP;
	if (mask) {
		PRINTM(MCMND, "Clear IES: 0x%x 0x%x 0x%x 0x%x 0x%x\n",
 ...
 */
static t_u16
woal_get_p2p_wps_ie(const t_u8 *ie, int len, t_u8 *ie_out)
{
	int left_len = len;
	const t_u8 *pos = ie;
	int length;
			    ...
			    t_u16 mask, t_u8 wait_option)
{
	...
	if (proberesp_ies_data) {
		if (proberesp_ies && proberesp_ies_len) {
			...
			proberesp_ies_data->ie_length =
				woal_get_p2p_wps_ie(proberesp_ies,
						    proberesp_ies_len,
						    proberesp_ies_data->
						    ie_buffer);
