...
struct usb_device_id driver_usb_table[] = {
	...
	{MARVELL_USB_DEVICE
	 (MARVELL_USB_VID, MARVELL_USB8997_Z_PID, "Marvell WLAN USB Adapter")},
	{MARVELL_USB_DEVICE
	 (MARVELL_USB_VID, MARVELL_USB8997_V2_PID, "Marvell WLAN USB Adapter")},
	/* Terminating entry */
	{},
};
...
driver_usb_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (udev->descriptor.idVendor == MARVELL_USB_VID &&
	    (udev->descriptor.idProduct == MARVELL_USB8766_PID ||
	     ...
	     udev->descriptor.idProduct == MARVELL_USB8997_Z_PID ||
	     udev->descriptor.idProduct == MARVELL_USB8997_V2_PID ||
		...
		else if (udev->descriptor.idProduct == MARVELL_USB8997_Z_PID)
			usb_cardp->usb_dev_type = DEV_TYPE_USB8997_Z;
		else if (udev->descriptor.idProduct == MARVELL_USB8997_V2_PID)
			usb_cardp->usb_dev_type = DEV_TYPE_USB8997_V2;
		else
			usb_cardp->usb_dev_type = DEV_TYPE_USB8897;
	} else {
		PRINTM(MERROR, "The Driver did not support this Device,"
		       "you can check whether your device is USB8766 or USB8797 or USB8997 or USB8897.\n");
		goto error;
	}
...
driver_update_fw_name(driver_handle * handle)
{
	...
	if (fw_name) {
		...
		} else if (handle->usb_dev_type == DEV_TYPE_USB8997_Z) {
			handle->fw_name = USB8997_Z_DEFAULT_FW_NAME;
		} else if (handle->usb_dev_type == DEV_TYPE_USB8997_V2) {
			handle->fw_name = USB8997_V2_DEFAULT_FW_NAME;
...
#define USB_WAIT_FW_READY  (500)
				...
				if (handle->usb_dev_type != DEV_TYPE_USB8997_Z
				    && handle->usb_dev_type !=
				    DEV_TYPE_USB8997_V2) {
